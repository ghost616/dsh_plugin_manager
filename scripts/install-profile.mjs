...
function rowsBlock() {
  return [
    ...
    '# Rows resolve relative to this profile directory. Single-row convergence:',
    '# plugin-market-host (package main) composes the market repository service',
    '# and activates the embedded control logic in the same context; the browser',
    '# half ships through the package dsh.client declaration, so there are no',
    '# separate control/ui loader rows. Remove everything with:',
    '#   node scripts/install-profile.mjs <profileDir> --uninstall',
    '- insert:',
